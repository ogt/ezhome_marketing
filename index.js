...
var api_key = ''; // Lob API key (Not used anymore, just for testing purposes)
...
exports.personalized = function(event, context) {
    var fs = require('fs'), Mustache = require('mustache');
    var Lob = require('lob')(api_key);

    fs.readFile('personalized_back.html', function (err, data) {
        if (err) {
            console.log("Failed to read personalized_back.html");
            context.fail(err);
            return;
        }

        var personalized_back = Mustache.render(data.toString(), event);

        fs.readFile('personalized_front.html', function (err, data) {
            if (err) {
                console.log("Failed to read personalized_front.html");
                context.fail(err);
                return;
            }

            var personalized_front = Mustache.render(data.toString(), event);
            ...
            Lob.postcards.create({
              ...
              setting: 1002,
              front: personalized_front,
              back: personalized_back
            }, function (err, res) {
                if (err != null) {
                    context.fail(err);
                } else {
...
exports.localized = function(event, context) {
    var fs = require('fs'), Mustache = require('mustache');
    var Lob = require('lob')(api_key);

    fs.readFile('localized_back.html', function (err, data) {
        if (err) {
            console.log("Failed to read localized_back.html");
            context.fail(err);
            return;
        }

        var localized_back = Mustache.render(data.toString(), event);

        fs.readFile('localized_front.html', function (err, data) {
            if (err) {
                console.log("Failed to read localized_front.html");
                context.fail(err);
                return;
            }

            var localized_front = Mustache.render(data.toString(), event);

            Lob.postcards.create({
              description: 'Localized Postcard',
              to: {
                name: 'Joe Smith',
                address_line1: '123 Main Street',
                address_city: 'Mountain View',
